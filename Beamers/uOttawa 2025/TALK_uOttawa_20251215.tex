...
\section{Newton-Schwarz methods} % 15 min

% phasefield
\begin{frame}{Brittle fracture}

\begin{figure}
\includegraphics[width=\textwidth]{FIG/FIG_BrittleFracture.png}
...
For subsequent iterations, we have to orthogonalize the vectors $\vec{d}_{\Gamma i}^{(k)}$:
	\begin{algorithmic}[1]
		...
		\For{$j=1:k-1$}
			...
			\State $\vec{v}_k \gets \vec{v}_k - h E_i^{(j)} \vec{d}_{\Gamma i}^{(j)}$ % nb: I think this line is unnecessary since E_i^{(k)} has d^{(j)} in its null space
		\EndFor
...
\For{j=1:k-1}
	...
	\State $\vec{w}_k \gets \vec{w}_k - h \vec{w}_j$
\EndFor
