...
%	\pause
	...
	\pause
	\node (bake02) at (0,0) {\includegraphics[width=0.75\textwidth, angle=-90]{FIG/bake02.jpg}};
\end{tikzpicture}
\end{figure}
...
\begin{tikzpicture}[every text node part/.style={align=left}]
	...
	\node[fill=UnigeGreen, text=white, shape=signal, signal from=west, signal to=east, minimum width=4*\yearlength, draw, right] at (8*\yearlength,0.5) {PhD};
	...
	\filldraw[fill=SFURed] (1*\yearlength,-1) circle (3pt) node[SFURed, thick, text=black, fill=white, right, draw] {\textbf{Mathematical Physics} \\ SFU};
	\filldraw (5*\yearlength,1.5) circle (3pt) node[SFURed, thick, text=black, left, draw] {\textbf{NSERC USRA} \\ SFU};
	\filldraw[fill=SFURed] (4*\yearlength,-2.5) circle (3pt) node[SFURed, thick, text=black, fill=white, right, draw] {\textbf{Applied Mathematics, NSERC CGS-M} \\ \textit{Pseudospectral methods} \\ SFU};
	\filldraw[fill=UnigeGreen] (11*\yearlength,2.5) circle (3pt) node[UnigeGreen, thick, text=black, left,draw] {\textbf{Applied Mathematics} \\ \textit{Domain decomposition} \\ UniGe};
	\filldraw[fill=uLavalGold] (11.5*\yearlength,2) circle (3pt) node[uLavalGold, thick, text=black, fill=white, right,draw] {\textbf{CRM-Laval PDF} \\ \textit{Adaptive Schwarz} \\ uLaval};
	\filldraw[fill=MacMaroon] (16*\yearlength,-1) circle (3pt) node[MacMaroon, thick, text=black, fill=white, left, draw] {\textit{Phasefield fracture} \\ McMaster};
\end{tikzpicture}
...
\begin{itemize}
\item 6 journal articles, 3 conference proceedings papers
\item 15 invited presentations at international conferences
\item Co-organized 2 minisymposia
\item 8 seminar presentations
\item Instructor for 5 courses
...
\item Adaptive optimized Schwarz methods
	...
	\item Equivalence to Krylov subspace methods
	\end{enumerate}
\end{enumerate}
\end{frame}
...
\begin{frame}
\frametitle{Project proposal: Parallelogram minimization}

\textbf{Project idea:} Extend minimization over the parallelogram to use a cubic polynomial instead.
Analyze minimization in several contexts where both a fixed point iteration and a Newton's method can be employed.
Can this be used more generally for all Newton-Schwarz methods?

\textbf{Possible collaborations:} Diane Guignard (stressed plates similar to phasefield model), Arian Novruzi (optimization)

\textbf{Mentoring opportunities:} This type of project marked the start of my PhD research.
The convergence of Newton's method and the need for globalization is well known and easy to grasp.
...
\end{equation*}
$E_i^{(k)} \vec{d}_{\Gamma i}^{(k)}$ is then the non-zero part of the right hand side of \eqref{eq: sym cells AOSM}.
...
\end{equation*}
For subsequent iterations, we have to orthogonalize the vectors $\vec{d}_{\Gamma i}^{(k)}$, i.e. with modified Gram-Schmidt.

If the matrix is symmetric, then the target transmission conditions are as well.
We want to choose updates that preserve that symmetry:
\begin{equation*}
	T_i^{(k+1)} := T_i^{(k)} - \frac{E_i^{(k)} \vec{d}_{\Gamma i}^{(k)} \left ( E_i^{(k)} \vec{d}_{\Gamma i}^{(k)} \right )^\top}{\left ( \vec{d}_{\Gamma i}^{(k)} \right )^\top E_i^{(k)} \vec{d}_{\Gamma i}^{(k)}} .
\end{equation*}
...
\begin{frame}
\frametitle{AOSM is equivalent to Krylov subspace methods}

\begin{block}{Equivalence theorem of general AOSM, CM. \& Kwok (2025)}
The AOSM for general matrices is equivalent to the full orthogonalization method (FOM).
\end{block}
...
\begin{block}{Convergence of AOSM for symmetric matrices}
The symmetric AOSM converges linearly and if the eigenvalues of $I -  \left ( \hat{A}_i + E_i^{(1)} \right )^{-1} E_i^{(1)}$ are clustered, then the AOSM converges superlinearly.
\end{block}

Unfortunately, for general matrices research tells us we can't know anything about the convergence of FOM or GMRES \textit{a priori}:
...
\textbf{Possible collaborations:} Currently proposed as a postdoctoral project with Chen Greif (UBC) and Manfred Trummer (SFU), could also involve Yves Bourgault.
...
\begin{frame}
\frametitle{Example: Laplace equation}

\begin{equation*}
	\begin{cases} \Delta u = -1 & x, y \in [-1,1] \\
	u(\pm1,y) = u(x,\pm1) = 0 \end{cases}
\end{equation*}

\begin{figure}
	\includegraphics[width=\textwidth]{FIG/MTLB_9blocks.png}
\end{figure}
...
AOSMs implement Krylov subspace methodology for faster convergence.
This implementation is more natural and more re-usable than traditional algorithms.
...
\item Saad, Y. \textbf{Iterative methods for sparse linear systems.} Society for Industrial and Applied Mathematics, 2003.
\item McCoid, C. \& Kwok, F. \textbf{Adaptive algebraic optimized Schwarz methods.} \textit{SIAM Journal on Matrix Analysis and Applications} 46 (2025).
\item Greenbaum, A., Ptak, V. \& Strakos, Z. \textbf{Any nonincreasing convergence curve is possible for GMRES.} \textit{SIAM Journal on Matrix Analysis and Applications} 17 (1996).
\end{itemize}
}
\end{frame}
...
\textbf{Possible collaborations:} Felix Kwok (uLaval).
I am also looking for PDEs to test these on.
Some thoughts: Frithjof Lutscher, Stacey Smith?, Steve Desjardins.
...
\section{Numerical results of AOSM on 2 subdomains}
...
\section{Numerical geometric intersections}

%===Paper-specific commands===%
% coordinates and signs
\newcommand{\pcoord}[2]{#1 \cdot \vec{e}_{#2}}
\newcommand{\qcoord}[2]{\vec{q}_{#2}^{#1}}
\newcommand{\xcoord}[2]{\pcoord{\vec{x}_{#1}}{#2}}
\newcommand{\wcoord}[2]{\vec{w}_{#2}^{#1}}
\newcommand{\signx}[2]{\sign(\xcoord{#1}{#2})}
\newcommand{\signq}[3]{\sign(\pcoord{\qcoord{#1}{#2}}{#3})}

\begin{frame}
\frametitle{Reference coordinate system}

Let the coordinates of the vertices of a clipping polytope $V$ be
\begin{equation*}
	\hat{V} = \begin{bmatrix} \vec{v}_0 & \vec{v}_1 & \dots & \vec{v}_n \end{bmatrix}.
\end{equation*}
This clipping polytope intersects a subject polytope $U$ with vertices with coordinates $\hat{U}$.

Transform both $V$ and $U$ such that $V$ has its facets aligned with cardinal hyperplanes:
\begin{equation*}
	\begin{bmatrix} \vec{v}_1 & \dots & \vec{v}_n \end{bmatrix} \hat{X} = \hat{U} - \vec{v}_0 \vec{1}^\top.
\end{equation*}

In addition to the $n$ Euclidean coordinates, add a 0th coordinate:
\begin{equation*}
	\vec{x} \cdot \vec{e}_0 = 1 - \sum_{i=1}^n \vec{x} \cdot \vec{e}_i.
\end{equation*}

\end{frame}

\begin{frame}
\frametitle{Binary sign and vertices inside $V$}

Use a binary-valued sign function to avoid degenerecies:
\begin{equation*}
	\sign(x) = \begin{cases} 1 & x \geq 0, \\ 0 & x < 0. \end{cases}
\end{equation*}

A vertex of $U$ lies in $V$ if its transformed coordinates satisfy
\begin{equation*}
	\chi(\vec{x}) = \prod_{i=0}^n \sign(\vec{x} \cdot \vec{e}_i) = 1.
\end{equation*}
...
If some of the vertices have different signs for some of these coordinates, then there are intersections.
Those intersections have coordinates:
\begin{equation*}
\pcoord{\qcoord{J}{\Gamma}}{\eta} = \frac{ \begin{vmatrix}
\xcoord{j_0}{\eta} & \xcoord{j_0}{i_1} & \dots & \xcoord{j_0}{i_k} \\
\vdots & \vdots & & \vdots \\
\xcoord{j_k}{\eta} & \xcoord{j_k}{i_1} & & \xcoord{j_k}{i_k} \end{vmatrix} }{ \begin{vmatrix}
1 & \xcoord{j_0}{i_1} & \dots & \xcoord{j_0}{i_k} \\
\vdots & \vdots & & \vdots \\
1 & \xcoord{j_k}{i_1} & & \xcoord{j_k}{i_k} \end{vmatrix} }
\end{equation*}

\end{frame}

\begin{frame}
\frametitle{Signs of intersection coordinates}

For robustness, we need the signs of the coordinates of intersections to be consistent with one another.

\begin{block}{Definition of $\otimes$}
	The operator $\otimes$ that acts as $0 \otimes 0 = 1$, $1 \otimes 1 = 1$ and $0 \otimes 1 = 0$ is called the logical biconditional operator.
\end{block}

\begin{block}{Lemma}
	Suppose $\signq{J_1}{\Gamma}{i} \neq \signq{J_2}{\Gamma}{i}$ and $\signq{J_1}{\Gamma}{j} \neq \signq{J_2}{\Gamma}{j}$ for two adjacent sets $J_1$ and $J_2$, then
	\begin{equation*}
		\signq{J_1 \cup J_2}{\Gamma \cup \set{i}}{j} \otimes \signq{J_1 \cup J_2}{\Gamma \cup \set{j}}{i} = \signq{J_1}{\Gamma}{i} \otimes \signq{J_2}{\Gamma}{j} .
	\end{equation*}
\end{block}
...
\begin{frame}
\frametitle{Straight line principle}

\begin{block}{Straight line principle}
Let $\qcoord{J}{\Gamma}$ and $\qcoord{K}{\Gamma}$ be two intersections such that $J$ is adjacent to $K$.
Suppose the line between them intersects $m \geq 3$ hyperplanes, i.e. $\signq{J}{\Gamma}{i} \neq \signq{K}{\Gamma}{i}$ for $m$ choices of $i$.
If
\begin{equation*}
	\signq{J \cup K}{\Gamma \cup \set{\gamma}}{i} = \signq{J}{\Gamma}{i}, \quad
	\signq{J \cup K}{\Gamma \cup \set{\gamma}}{j} \neq \signq{J}{\Gamma}{j},
\end{equation*}
then
\begin{align*}
	\signq{J \cup K}{\Gamma \cup \set{j}}{i} = \signq{J}{\Gamma}{i}, \\
	\signq{J \cup K}{\Gamma \cup \set{i}}{j} = \signq{K}{\Gamma}{j} \neq \signq{J}{\Gamma}{j}.
\end{align*}
\end{block}
...
\begin{figure}
	\centering
	\begin{tikzpicture}
		\coordinate (i0) at (-3,0);
		\coordinate (i1) at (1,0);
		\coordinate (j0) at (0,-1);
		\coordinate (j1) at (0,3);
		\coordinate (k0) at (1,-1);
		\coordinate (k1) at (-3,3);
		\draw[thick,->,black] (i0)--(i1) node[right] {$i$};
		\draw[thick,->,black] (j0)--(j1) node[right] {$j$};
		
		\node (qJ) at (-3,-1) {$\qcoord{J}{\Gamma}$};
		\node (qK) at (1,2) {$\qcoord{K}{\Gamma}$};
		\draw[thick,dashed,red] (qJ)--(qK);
		\filldraw[blue] (intersection of i0--i1 and qJ--qK) circle (2pt) node[below right, black] {$\qcoord{J \cup K}{\Gamma \cup \set{j}}$};
		\filldraw[blue] (intersection of j0--j1 and qJ--qK) circle (2pt) node[below right, black] {$\qcoord{J \cup K}{\Gamma \cup \set{i}}$};
		\filldraw[blue] (intersection of k0--k1 and qJ--qK) circle (2pt) node[above left, black] {$\qcoord{J \cup K}{\Gamma \cup \set{\gamma}}$};
	\end{tikzpicture}
\end{figure}
...
\begin{frame}
\frametitle{More intersections to consider}

\begin{itemize}
\item 2D-3D, and other mixed dimension
\item More general polytopes (non-simplex)
\item Non-convex objects
\item Combinatorics of intersections
\end{itemize}
